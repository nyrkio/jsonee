...
InMemoryStore — pure Python, snapshot-to-disk optional. Used by tests and
    short-lived processes; pass ``store=InMemoryStore()`` to build_app().

MongoStore — thin pymongo wrapper that speaks the MongoDB wire protocol.
    Works with secantusdb (embedded), real MongoDB, and FerretDB.
...
open_store(storage_path, uri) — factory used by build_app() when no store
    is supplied:
      ...
      • Otherwise → embedded secantusdb at storage_path
        (defaults to ":memory:" — ephemeral, same behaviour as old
        InMemoryStore() default; pass a real directory for persistence)
"""
import os
import threading
...
class _MongoCollection:
    """Adapts a pymongo Collection to the same interface as _Collection."""
